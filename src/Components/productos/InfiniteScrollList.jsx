  ...
  return (
    <div className='mt-20'>
      <ul className='grid grid-cols-1 gap-4 md:grid-cols-2 xl:grid-cols-3 '>
        ...
        <h5 className="mb-2 text-2xl font-bold tracking-tight text-gray-900 dark:text-white">{item.title}1</h5>
        <h6 className="mb-2 text-l font-bold tracking-tight text-gray-900 dark:text-white">Categoria: {item.category}1</h6>
        <p className="mb-3 font-normal text-gray-700 dark:text-gray-400">Price:${item.price}</p>
        {/* <p className="mb-3 font-normal text-gray-700 dark:text-gray-400">{item.description}</p> */}
        <AgregarCarrito  valoritem={item} valorid = {item.id} />
        </div>
      </div>
          </li>
        ))}
